  ...
  if (isAnimating) return;
    ...
    if (scrollDirection === 'down') {
      if (index === 0) {
        ...
        animateElement(CONTAINER_SOUS_ACCUEIL, '-55%', 1);
      } else if (index === 1) {
      ...
      if(index === 0){
        ...
        animateElement(DIV_TEST_SELECTORS[0], '100%', 0, () => {
        ...
        animateElement(DIV_TEST_SELECTORS[3], '100%', 1);
...
function handleScrollEventCircle(startY, endY){
  ...
  if (scrollDirection === 'down') {
    ...
    if(indexCircle == 0){
      isAnimating = true;
      gsap.to("#cover", {duration: 1, attr:{r:'71%'}, onComplete: () => {
        isAnimating = false; // Réinitialiser isAnimating à false une fois l'animation terminée
